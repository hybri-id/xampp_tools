...
After searching in the net for a consistent mySQL backup tool for XAMPP I ended doing my own tool because I haven't found something that fits my needs. This tool backups all mySQL databases of XAMPP directory, then compress them in .gz format to save disk space. You can also schedule the backups and restore them with only few key presses. It has many advantages because the scripts are modular.
